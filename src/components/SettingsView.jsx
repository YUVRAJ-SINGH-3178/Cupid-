...
const SettingToggle = ({ label, desc, active, onToggle }) => (
    <div className="flex justify-between items-center py-4 border-b border-border last:border-0 group gap-4">
        <div className="flex-1 min-w-0">
            <h4 className="text-xs sm:text-sm font-bold text-foreground group-hover:text-vibe-purple transition-colors">{label}</h4>
            {desc && <p className="text-[11px] sm:text-xs text-foreground-muted mt-1">{desc}</p>}
        </div>
        <button
            onClick={onToggle}
            className={cn("w-12 h-7 rounded-full relative transition-colors duration-300 flex-shrink-0", active ? "bg-vibe-purple" : "bg-card-hover")}
        >
...
export const SettingsView = ({ currentUser, onLogout, onUpdateProfile }) => {
    ...
    return (
        <motion.div
            ...
            className="pb-24 lg:pb-8 max-w-3xl mx-auto space-y-6 lg:space-y-8 px-3 sm:px-4"
        >
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 mb-6 sm:mb-8 obsidian-card p-6 sm:p-8 rounded-[2.5rem] border border-border relative overflow-hidden group">
                ...
                <div className="relative group/avatar flex-shrink-0">
                    ...
                    <img
                        ...
                        alt="Profile"
                        className="w-20 sm:w-24 h-20 sm:h-24 rounded-full border-4 border-[#050505] relative z-10 bg-[#1a1a1f] object-cover"
                    />
                ...
                <div className="relative z-10 text-center sm:text-left flex-1">
                    <h1 className="text-2xl sm:text-3xl font-display font-bold text-foreground tracking-tight">{safeUser.full_name || safeUser.username}</h1>
                    <p className="text-foreground-muted text-xs sm:text-sm mt-1 font-medium">@{safeUser.username} • <span className="text-foreground-muted">Student</span></p>
                    <div className="flex gap-2 mt-4 justify-center sm:justify-start flex-wrap">
                        <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[9px] sm:text-[10px] font-bold rounded-lg border border-emerald-500/20 flex items-center gap-1.5 shadow-sm">
                            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_5px_#10b981]" /> Online
                        </span>
                        <span className="px-3 py-1 bg-amber-500/10 text-amber-500 text-[9px] sm:text-[10px] font-bold rounded-lg border border-amber-500/20 flex items-center gap-1.5">
                            Pro Member
                        </span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 mb-6">
                    {[
                        ...
                        <button
                            ...
                            className={cn(
                                "p-3 rounded-xl border transition-all flex flex-col items-center gap-2 group relative overflow-hidden",
                                theme === t.id
                        ...
                        >
                            <t.icon className={cn("w-5 sm:w-6 h-5 sm:h-6 transition-transform group-hover:scale-110 duration-300", theme === t.id && "text-vibe-purple")} />
                            <span className="capitalize text-[10px] sm:text-xs font-bold tracking-wide text-center line-clamp-2">{t.label}</span>
            ...
            <div className="pt-2 px-3 sm:px-0">
                ...
                <div className="flex flex-col items-center mt-6 gap-2">
                    <p className="text-[9px] sm:text-[10px] text-gray-600 uppercase tracking-[0.2em] font-bold">Cupid v4.2.0-beta</p>
                    <p className="text-[8px] sm:text-[10px] text-gray-700 font-mono">Build 2024.11.14_rc2</p>
                </div>
            </div>
